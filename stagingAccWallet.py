...
class TestAccountWallet:
    ...
    def url(self):
        ...
        walletSettlAPIResp = requests.get(
            "https://lendittfinserve.com/stag/admin/tally/getWalletSettlementDetails?startDate=2023-10-05T10%3A00%3A00.000Z&endDate=2023-10-05T10%3A00%3A00.000Z")  # current date

        '''# RAZORPAY-1'''
        razPay1 = requests.get("https://api.razorpay.com/v1/settlements",
                               params={"from":1672531200,"to":1672617600},
        ...
        data = {"pagination": {"limit": 100},
                "filters": {"start_date": "2023-10-05T18:30:00+05:30",
                            "end_date": "2023-10-05T18:30:00+05:30"}}  # start date = previous date, end date = current date
    ...
    def test_postCashfreeSettl(self,url):
        ...
        if amtReceivedInCashFreeDebitTotalFloat == amtSettlList:
            print("walletCashFreeCreditTotalFloat matched with totalOfAmtSettl")
        else:
            print("walletCashFreeCreditTotalFloat not matched with totalOfAmtSettl")
